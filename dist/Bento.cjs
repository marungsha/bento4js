...
class Bento4 {
    static transcode() {
        return __awaiter(this, arguments, void 0, function* (files = [], options = { aesKey: null, noIframe: false, singleFile: false, outputPath: "output" }, onComplete, onError = (error) => { }) {
            /*
            ...
            */
            ...
            if (!fs_extra_1.default.existsSync(targetFolder.join('/')))
                fs_extra_1.default.mkdirSync(targetFolder.join('/'));
            // Final folder will be created by bento
            if (fs_extra_1.default.existsSync(options.outputPath))
                fs_extra_1.default.rmdirSync(options.outputPath);
    ...
    static getEncryptionKey(key, cb = (_) => { }) {
    ...
    static runEncode(files, options, onComplete = (__, _) => { }, onError = (_) => { }) {
        // prepare arguments
        let args = [];
        if (options.aesKey)
    ...
    static removeIframe(options) {
        ...
        let test = fs_extra_1.default.readFileSync(options.outputPath + "/master.m3u8", { encoding: 'utf-8' });
        //console.log(test.split('# I-Frame Playlists')[0])
        fs_extra_1.default.writeFileSync(options.outputPath + "/master.m3u8", test.split('# I-Frame Playlists')[0], { encoding: 'utf-8' });
    }
}
